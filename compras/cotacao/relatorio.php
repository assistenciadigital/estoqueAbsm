...
include("../../includes/conect.php");
	...
	$select = mysql_query("SELECT * FROM historicomovimento where local_id = '".$_GET['local']."' order by historicomovimento_data ASC");	
	}else if ($_GET['id']){
	$select = mysql_query("SELECT * FROM historicomovimento where historicomovimento_id = '".$_GET['id']."' order by historicomovimento_data ASC");	
	}else{
...
if ($_GET['local']){
	$local = @mysql_fetch_assoc (mysql_query ("SELECT localestoque_descricao FROM local where local_id ='".$_GET['local']."'"));
	$local = $depto['local_descricao'];
}else{
	$local = "*";
}

$pdf-> SetFont("Courier", 'B', 8);
$pdf-> Cell(0,4, number_format(($qtde_pesquisado), 0, ',', '.').' de '.number_format(($qtde_registro), 0, ',', '.')." Registro(s) [Local:".$local."]",1,1,'C');
$pdf->SetFont("Courier", '', 8);
...
?> 
<script>window.open('cotacao.pdf'); </script>