...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
	...
	$pedido = @mysql_fetch_assoc(mysql_query ("SELECT pedido.fornecedor, pedido.id, pedido.data_pedido, fornecedor.fornecedor_razaosocial from pedido inner join fornecedor on pedido.fornecedor = fornecedor.fornecedor_id where pedido.id = '".$_GET['pedido_id']."'"));		
...
<form onsubmit="return validacampo(this)" action="continua_pedido.php?pedido_id=<?php echo $_GET['pedido_id']?>" name="frmcadastro" id="frmcadastro"  method="post">
  <div style="border: 1px solid #c30; background: #ffe; font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:14px" align="center"><strong><?php echo 'COTAÇÃO / PEDIDO DE COMPRAS: '.$_GET['pedido_id'].' - '.$pedido['fornecedor_razaosocial'].' - '.date_data($pedido['data_pedido'])?></strong>
        </div><p>
  ...
  <div style="border: 1px solid #c30; background: #ffe; font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:14px" align="center">
    <p><strong>COTAÇÃO / PEDIDO DE COMPRAS</strong> (Digite o que você está procurando e clique em Pesquisar)<p><input name="pesquisa" type="text" id="pesquisa" value="<?php echo $_POST['pesquisa']?>" size="50" maxlength="50" />&nbsp;<input type="submit" name="pesquisar" id="pesquisar" value="Pesquisar..." />
<input type="reset" name="limpar" id="limpar" value="Limpar..." onclick="limpa()"/>
<a href="pedido.php"><input type="button" name="voltar" id="voltar" value="Voltar..." /></a><p></div><p>
</form>
<?php if ($select['produto_id']){?>
<form action="processa.php?acao=continua_pedido" method="post" name="frmprocessa" id="frmprocessa">
<div style="border: 1px solid #c30; background: #ffe; font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:11px" align="center">
  ...
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0"  class="pesquisa">
      <tr>
        <td width="10%" align="right" valign="middle" bgcolor="#CCCCCC">Pedido:</td>
        <td width="20%" align="left" valign="middle" bgcolor="#CCCCCC">
          <input name="pedido_id" type="text" id="pedido_id" style="border:transparent; background:transparent" value="<?php echo $_GET['pedido_id']?>" size="20" maxlength="20" readonly="readonly" />
          Id Produto:
      ...
      <tr>
        <td width="10%" align="right" valign="middle" bgcolor="#CCCCCC">Local Estoque:</td>
      ...
      <tr>
        <td width="10%" align="right" valign="middle" bgcolor="#CCCCCC">&nbsp;</td>
        <td width="20%" align="left" valign="middle" bgcolor="#CCCCCC">&nbsp;</td>
        ...
        <td width="20%" align="left" valign="middle" bgcolor="#CCCCCC"><strong>
          ...
          COTAÇÃO / PEDIDO DE COMPRAS</strong></td>
        </tr>
      <tr>
        <td align="right" valign="middle" bgcolor="#CCCCCC"><p>Preço de Custo R$:</p></td>
        <td width="20%" align="left" valign="middle" bgcolor="#CCCCCC"><input name="preco_custoatual" type="text" onkeypress="return(adjustMoney(event,this,',','.'))"  id="preco_custoatual" onblur="calculo()"/>
        % Custo:
        <input name="percentualcusto" type="text" class="frm_number_only" id="percentualcusto" value="20" size="5" maxlength="5" onblur="calculo()"/>
        </td>
        <td align="right" valign="middle" bgcolor="#CCCCCC">Preço de Venda R$:</td>
        <td width="20%" align="left" valign="middle" bgcolor="#CCCCCC"><strong>
          <input name="produto_preco" type="text" onkeypress="return(adjustMoney(event,this,',','.'))"  id="produto_preco" />
          <strong>
          <input name="calculoprecovenda" type="text" disabled="disabled" id="calculoprecovenda" style="background:transparent; border:transparent" onkeypress="return(adjustMoney(event,this,',','.'))" readonly="readonly" />
          </strong>        </strong></td>
        </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="3" align="left" valign="middle" bgcolor="#CCCCCC"><input name="historico" type="text" id="historico" value="<?php echo "COTACAO / PEDIDO DE COMPRAS - EM: ".date('d/m/Y H:m')." USUARIO: ".$_SESSION['login'] ?>" size="100" /></td>
        </tr>
      <tr>
