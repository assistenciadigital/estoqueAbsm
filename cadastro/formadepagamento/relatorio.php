...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from formadepagamento where formadepagamento_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from formadepagamento where formadepagamento_id like '%".$_GET['id']."%' and formadepagamento_descricao like '%".$_GET['descricao']."%' order by formadepagamento_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from formadepagamento"));
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['formadepagamento_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['formadepagamento_descricao'], 1, 0, 'L');
...
$pdf->Output("formadepagamento.pdf", "I");
?> 
<script>window.open('formadepagamento.pdf'); </script>