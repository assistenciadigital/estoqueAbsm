...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from status where status_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from status where status_id like '%".$_GET['id']."%' and status_descricao like '%".$_GET['descricao']."%' order by status_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from status"));
...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'RELACAO DE STATUS',0,1,'C');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['status_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['status_descricao'], 1, 0, 'L');
...
$pdf->Output("status.pdf", "I");
?> 
<script>window.open('status.pdf'); </script>