...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
    ...
    $select = @mysql_fetch_assoc(mysql_query("SELECT * FROM status where status_id = '".$_POST['id']."'"));
...
<?php
	...
	if(($_SESSION['nivel'])<=2){
			mysql_query ("DELETE FROM status WHERE status_id='".$_GET['id']."'");
...
//------------------------------------->> ALTERAR	
	...
	if(($_SESSION['nivel'])<=2){
	if (!(mysql_num_rows (mysql_query ("SELECT status_id FROM status WHERE status_descricao = '".$_POST['descricao']."'")))) { //VERIFICA DUPLICIDADE
			$altera = mysql_query("UPDATE status SET status_descricao='".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', data='".date('Y-m-d H:i:s')."', usuario_login='".$_SESSION['login']."' where status_id='".$_POST['id']."'");
...
//------------------------------------->> INCLUIR
		...
		if (!(mysql_num_rows (mysql_query ("SELECT status_id FROM status WHERE status_descricao='".$_POST['descricao']."'")))) { // VERFICA DUPLICIDADE
			$insere = mysql_query ("INSERT status(status_descricao, data, usuario_login) VALUES('".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', '".date('Y-m-d H:i:s')."', '".$_SESSION['login']."')");
