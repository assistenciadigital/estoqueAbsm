...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from cor_raca where cor_raca_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from cor_raca where cor_raca_id like '%".$_GET['id']."%' and cor_raca_descricao like '%".$_GET['descricao']."%' order by cor_raca_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from cor_raca"));
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['cor_raca_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['cor_raca_descricao'], 1, 0, 'L');
...
$pdf->Output("cor_raca.pdf", "I");
?> 
<script>window.open('cor_raca.pdf'); </script>