...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
	...
	if ($_POST['classificacao'] and $_POST['id']){
	$select = mysql_query("SELECT * FROM tipo where classificacao_id = '".$_POST['classificacao']."' and tipo_id = '".$_POST['id']."'");
	}else{
	$select = mysql_query("SELECT * FROM tipo where classificacao_id like '%".$_POST['classificacao']."%' and tipo_id like '%".$_POST['id']."%' and tipo_descricao like '%".$_POST['descricao']."%' order by tipo_descricao");	
	}
	
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro = mysql_num_rows(mysql_query("SELECT * from tipo")); // Quantidade de registros pra paginação
...
function exclui(tipo_id){
	if(confirm ('Confirma a exclusão do registro: ' + tipo_id + '?')) {
		location = 'processa.php?acao=exclusao&id=' + tipo_id;
	}
}
...
// Captura o retorno do retornaCliente.php
    $.getJSON('../../includes/autocomplete_retorna_tipo.php', function(data){
    var dados = [];
    // Armazena na array capturando somente o nome do EC
    $(data).each(function(key, value) {
        dados.push(value.tipo_descricao);
    });
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <th width="100%" colspan="2" align="center" scope="col"><div align="midle" style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">CONSULTA TIPO</div></th>
  </tr>
  <tr>
...
<form action="lista.php" name="frmlista" id="frmlista" method="post" enctype="multipart/form-data">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">    
    <tr bgcolor="#CCCCCC" style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
      ...
      <td width="80%" align="center" valign="middle" bgcolor="#CCCCCC" style="cursor:pointer" scope="col"><strong>Filtra Classificacao: 
          <select name="classificacao" id="classificacao" style="width:100px; height: 20px;" onchange="atualiza();">
            <option value=""></option>
            <?php
            $select_classificacao = mysql_query ("SELECT classificacao_id,classificacao_descricao from classificacao ORDER BY classificacao_id");
            if (mysql_num_rows ($select_classificacao)) {
            while ($row_classificacao= mysql_fetch_assoc($select_classificacao)) {
            ?>
            <option value="<?php echo $row_classificacao['classificacao_id']?>"<?php if($_POST['classificacao'] == $row_classificacao['classificacao_id']) echo "selected"?>><?php echo $row_classificacao['classificacao_id'].' - '.$row_classificacao['classificacao_descricao']?></option>
            <?php }}?>
          </select>Filtra ID: 
          <select name="id" id="id" style="width:200px; height: 20px;" onchange="atualiza();">
            <option value=""></option>
            <?php
            $select_tipo = mysql_query ("SELECT tipo_id, tipo_descricao from tipo where classificacao_id='".$_POST['classificacao']."' ORDER BY tipo_id");
            if (mysql_num_rows ($select_tipo)) {
            while ($row_tipo= mysql_fetch_assoc($select_tipo)) {
            ?>
            <option value="<?php echo $row_tipo['tipo_id']?>"<?php if($_POST['id'] == $row_tipo['tipo_id']) echo "selected"?>><?php echo $row_tipo['tipo_id'].' - '.$row_tipo['tipo_descricao']?></option>
            <?php }}?>
          </select>
          Filtra  Descrição:&nbsp;
      ...
      <td width="10%" align="right" valign="middle" bgcolor="#CCCCCC" style="cursor:pointer" scope="col"<?php if(($_SESSION['nivel'])<=2){ ?><?php } ?>><strong>
        <?php if(($_SESSION['nivel'])<=2){ ?>
        <a href="relatorio.php?classificacao=<?php echo $_POST['classificacao']?>&id=<?php echo $_POST['id']?>&descricao=<?php echo $_POST['descricao']?>" target="_blank"><input type="button" name="impressao" id="impressao" value="Impressão..." style="cursor:pointer"/></a>
        <?php } ?>
      </strong></td>
      </tr>
  </table>
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
      <td width="10%" align="center" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'"><?php echo $row['tipo_id']?></td>
      <td width="80%" align="rigth"  bgcolor="<?php echo $cor?>" scope="col" onMouseOver="this.style.backgroundColor='#00FFFF'" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'"><?php echo $row['tipo_descricao']?></td>
      <td width="5%" align="center" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'" style="cursor:pointer"><?php if(($_SESSION['nivel'])<=2){ ?><a href="cadastra.php?id=<?php echo $row['tipo_id']?>"><img src="../../imgs/editar.png" width="20" height="20" /></a><?php } ?></td>
      <td width="5%" align="center" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'" style="cursor:pointer"><?php if(($_SESSION['nivel'])<=2){ ?><a href="javascript: exclui (<?php echo $row['tipo_id']?>)"> <img src="../../imgs/excluir.png" width="20" height="20" /></a><?php } ?></td>
    </tr>
  </table>
  <?php }}?>
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
