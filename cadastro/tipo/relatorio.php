...
//CONECTA COM O MYSQL
if ($_GET['classificacao'] and $_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from tipo where classificacao_id like '".$_GET['classificacao']."' and tipo_id = '".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from tipo where classificacao_id like '%".$_GET['classificacao']."%' and tipo_id like '%".$_GET['id']."%' and tipo_descricao like '%".$_GET['descricao']."%' order by tipo_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from tipo"));
...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'RELACAO DE TIPO',0,1,'C');
	  ...
	  //MONTA O CABEÇALHO DA TABELA
		...
		$this->Cell(81, 4, "DESCRICAO", 1, 0, 'L');
		$this->Cell(41, 4, "CLASSIFICACAO", 1, 0, 'L');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
		$classifica = @mysql_fetch_assoc (mysql_query ("SELECT classificacao_descricao FROM classificacao where classificacao_id ='".$row['classificacao_id']."'"));
	
	$pdf->Cell( 14, 4, $row['tipo_id'], 1, 0, 'C');
	$pdf->Cell( 81, 4, $row['tipo_descricao'], 1, 0, 'L');
	$pdf->Cell( 41, 4, $classifica['classificacao_descricao'], 1, 0, 'L');
...
if ($_GET['classificacao']){
	$classificacao = $classifica['classificacao_descricao'];
}else{
	$classificacao = "*";
}
...
$pdf-> Cell(0,4, number_format(($qtde_pesquisado), 0, ',', '.').' de '.number_format(($qtde_registro), 0, ',', '.')." Registro(s) [Classificacao: ".$classificacao." ID: ".$id." Descricao Contendo: ".$descricao."]",1,1,'C');
$pdf->SetFont("Courier", '', 8);
...
$pdf->Output("tipo.pdf", "I");
?> 
<script>window.open('tipo.pdf'); </script>