...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
    ...
    $select = @mysql_fetch_assoc(mysql_query("SELECT * FROM tipo where tipo_id = '".$_POST['id']."'"));
...
<?php
	...
	if(($_SESSION['nivel'])<=2){
			mysql_query ("DELETE FROM tipo WHERE tipo_id='".$_GET['id']."'");
...
//------------------------------------->> ALTERAR	
	...
	if(($_SESSION['nivel'])<=2){
	if (!(mysql_num_rows (mysql_query ("SELECT tipo_id FROM tipo WHERE tipo_descricao = '".$_POST['descricao']."' and classificacao_id = '".$_POST['classificacao']."'")))) { //VERIFICA DUPLICIDADE
			$altera = mysql_query("UPDATE tipo SET tipo_descricao='".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', classificacao_id='".$_POST['classificacao']."', data='".date('Y-m-d H:i:s')."', usuario_login='".$_SESSION['login']."' where tipo_id='".$_POST['id']."'");
...
//------------------------------------->> INCLUIR
		...
		if (!(mysql_num_rows (mysql_query ("SELECT tipo_id FROM tipo WHERE tipo_descricao='".$_POST['descricao']."' and classificacao_id = '".$_POST['classificacao']."'")))) { // VERFICA DUPLICIDADE
			$insere = mysql_query ("INSERT tipo(tipo_descricao, classificacao_id, data, usuario_login) VALUES('".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', '".$_POST['classificacao']."', '".date('Y-m-d H:i:s')."', '".$_SESSION['login']."')");
