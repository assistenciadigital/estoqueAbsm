...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
	...
	if ($_GET['id']) $titulo = "ALTERANDO TIPO";
				else $titulo = "CADASTRANDO TIPO";
		
	$select = @mysql_fetch_assoc(mysql_query("SELECT * FROM tipo where tipo_id = '".$_GET['id']."'"));
    ...
    <table width="100%" border="0" align="center" cellpadding="3" cellspacing="3">
      <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
        ...
        <td width="80%" align="left" valign="middle" bgcolor="#CCCCCC"><label for="id"></label>
          <input style="border:transparent; background:transparent" name="id" type="text" id="id" value="<?php if ($_GET['id']) echo $select['tipo_id']; else echo 'registro novo';?>" size="20" maxlength="20" readonly="readonly" /></td>
        </tr>
      <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
        <td width="20%" align="right" valign="middle" bgcolor="#CCCCCC"><strong>Descrição:</strong></td>
        <td width="80%" align="left" valign="middle" bgcolor="#CCCCCC"><input name="descricao" type="text" id="descricao" value="<?php echo $select['tipo_descricao']?>" size="60" maxlength="60" /></td>
        </tr>
      <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
        <td align="right" valign="middle" bgcolor="#CCCCCC"><strong>Classificação:</strong></td>
        <td align="left" valign="middle" bgcolor="#CCCCCC"><select name="classificacao" id="classificacao" style="width:250px; height: 20px;">
          <option value="">Classificacao</option>
          <?php
            $select_classificacao = mysql_query ("SELECT * from classificacao ORDER BY classificacao_descricao");
            if (mysql_num_rows ($select_classificacao)) {
            while ($row_classificacao = mysql_fetch_assoc($select_classificacao)) {
            ?>
          <option value="<?php echo $row_classificacao['classificacao_id']?>"<?php if($select['classificacao_id'] == $row_classificacao['classificacao_id']) echo "selected"?>><?php echo $row_classificacao['classificacao_descricao']?></option>
          <?php }}?>
        </select></td>
      </tr>
      <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
