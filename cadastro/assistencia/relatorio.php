...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from assistencia where assistencia_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from assistencia where assistencia_id like '%".$_GET['id']."%' and assistencia_descricao like '%".$_GET['descricao']."%' order by assistencia_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from assistencia"));
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['assistencia_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['assistencia_descricao'], 1, 0, 'L');
...
$pdf->Output("assistencia.pdf", "I");
?> 
<script>window.open('assistencia.pdf'); </script>