...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
    ...
    $select = @mysql_fetch_assoc(mysql_query("SELECT * FROM assistencia where assistencia_id = '".$_POST['id']."'"));
...
<?php
	...
	if(($_SESSION['nivel'])<=2){
			mysql_query ("DELETE FROM assistencia WHERE assistencia_id='".$_GET['id']."'");
...
//------------------------------------->> ALTERAR	
	...
	if(($_SESSION['nivel'])<=2){
	if (!(mysql_num_rows (mysql_query ("SELECT assistencia_id FROM assistencia WHERE assistencia_descricao = '".$_POST['descricao']."'")))) { //VERIFICA DUPLICIDADE
			$altera = mysql_query("UPDATE assistencia SET assistencia_descricao='".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', data='".date('Y-m-d H:i:s')."', usuario_login='".$_SESSION['login']."' where assistencia_id='".$_POST['id']."'");
...
//------------------------------------->> INCLUIR
		...
		if (!(mysql_num_rows (mysql_query ("SELECT assistencia_id FROM assistencia WHERE assistencia_descricao='".$_POST['descricao']."'")))) { // VERFICA DUPLICIDADE
			$insere = mysql_query ("INSERT assistencia(assistencia_descricao, data, usuario_login) VALUES('".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', '".date('Y-m-d H:i:s')."', '".$_SESSION['login']."')");
