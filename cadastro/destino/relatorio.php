...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from destino where destino_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from destino where destino_id like '%".$_GET['id']."%' and destino_descricao like '%".$_GET['descricao']."%' order by destino_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from destino"));
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['destino_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['destino_descricao'], 1, 0, 'L');
...
$pdf->Output("destino.pdf", "I");
?> 
<script>window.open('destino.pdf'); </script>