...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from ocupacao where ocupacao_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from ocupacao where ocupacao_id like '%".$_GET['id']."%' and ocupacao_descricao like '%".$_GET['descricao']."%' order by ocupacao_descricao");
}
	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from ocupacao"));
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['ocupacao_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['ocupacao_descricao'], 1, 0, 'L');
...
$pdf->Output("ocupacao.pdf", "I");
?> 
<script>window.open('ocupacao.pdf'); </script>