...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from acomodacao where acomodacao_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from acomodacao where acomodacao_id like '%".$_GET['id']."%' and acomodacao_descricao like '%".$_GET['descricao']."%' order by acomodacao_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from acomodacao"));
...
class PDF extends FPDF
{
   ...
   function Footer() {
      ...
      //além de informar se terá borda e o alinhamento do texto
  	  $this->Cell(0,0,'',1,1,'C');
	  $this->Cell(0,2,'acomodacao Digital - Telefone: 55(65)9606-2605 - Todos os direitos reservados.',0,1,'C');
	  $this->Cell(0,2,'Website: http://www.acomodacaodigital.info | E-mail: alex@acomodacaodigital.info',0,0,'C');
   }
}
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['acomodacao_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['acomodacao_descricao'], 1, 0, 'L');
...
$pdf->Output("acomodacao.pdf", "I");
?> 
<script>window.open('acomodacao.pdf'); </script>