...
//CONECTA COM O MYSQL
if ($_GET['ordem']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from patente where patente_ordem='".$_GET['ordem']."'");
}else if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from patente where patente_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from patente where patente_ordem like '%".$_GET['ordem']."%' and patente_id like '%".$_GET['id']."%' and patente_descricao like '%".$_GET['descricao']."%' order by patente_ordem");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from patente"));
...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'RELACAO DE PATENTE',0,1,'C');
	  ...
	  //MONTA O CABEÇALHO DA TABELA
		...
		$this->Cell( 11, 4, "ORDEM", 1, 0, 'C');
		$this->Cell( 50, 4, "DESCRICAO", 1, 0, 'L');
		$this->Cell( 61, 4, "DETALHE", 1, 0, 'L');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['patente_id'], 1, 0, 'C');
	$pdf->Cell(11, 4,  $row['patente_ordem'], 1, 0, 'C');
	$pdf->Cell(50, 4,  $row['patente_descricao'], 1, 0, 'L');
	$pdf->Cell(61, 4,  $row['patente_detalhe'], 1, 0, 'L');
...
$pdf-> Cell(0,4, number_format(($qtde_pesquisado), 0, ',', '.').' de '.number_format(($qtde_registro), 0, ',', '.')." Registro(s) [Ordem: ".$ordem." ID: ".$id." Descricao Contendo: ".$descricao."]",1,1,'C');
$pdf->SetFont("Courier", '', 8);
...
$pdf->Output("patente.pdf", "I");
?> 
<script>window.open('patente.pdf'); </script>