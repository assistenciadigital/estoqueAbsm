...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
	...
	if ($_GET['id']) $titulo = "ALTERANDO ESPECIALIDADE";
				else $titulo = "CADASTRANDO ESPECIALIDADE";
		
	$select = @mysql_fetch_assoc(mysql_query("SELECT * FROM especialidade where especialidade_id = '".$_GET['id']."'"));
    ...
    <table width="100%" border="0" align="center" cellpadding="3" cellspacing="3">
      <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
        ...
        <td width="80%" align="left" valign="middle" bgcolor="#CCCCCC"><label for="id"></label>
          <input style="border:transparent; background:transparent" name="id" type="text" id="id" value="<?php if ($_GET['id']) echo $select['especialidade_id']; else echo 'registro novo';?>" size="20" maxlength="20" readonly="readonly" /></td>
        </tr>
      <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
        <td width="20%" align="right" valign="middle" bgcolor="#CCCCCC"><strong>Descrição:</strong></td>
        <td width="80%" align="left" valign="middle" bgcolor="#CCCCCC"><input name="descricao" type="text" id="descricao" value="<?php echo $select['especialidade_descricao']?>" size="60" maxlength="60" /></td>
        </tr>
      <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
        <td align="right" valign="middle" bgcolor="#CCCCCC"><strong>Área de Atuação:</strong></td>
        <td align="left" valign="middle" bgcolor="#CCCCCC"><select name="especialidade" id="especialidade" style="width:250px; height: 20px;">
          <option value="">Área de atuação</option>
          <?php
            $select_especialidade = mysql_query ("SELECT distinct especialidade_tipo from especialidade ORDER BY especialidade_tipo");
            if (mysql_num_rows ($select_especialidade)) {
            while ($row_especialidade = mysql_fetch_assoc($select_especialidade)) {
            ?>
          <option value="<?php echo $row_especialidade['especialidade_tipo']?>"<?php if($select['especialidade_tipo'] == $row_especialidade['especialidade_tipo']) echo "selected"?>><?php echo $row_especialidade['especialidade_tipo']?></option>
          <?php }}?>
        </select></td>
      </tr>
      <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
