...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
    ...
    $select = @mysql_fetch_assoc(mysql_query("SELECT * FROM origem where origem_id = '".$_POST['id']."'"));
...
<?php
	...
	if(($_SESSION['nivel'])<=2){
			mysql_query ("DELETE FROM origem WHERE origem_id='".$_GET['id']."'");
...
//------------------------------------->> ALTERAR	
	...
	if(($_SESSION['nivel'])<=2){
	if (!(mysql_num_rows (mysql_query ("SELECT origem_id FROM origem WHERE origem_descricao = '".$_POST['descricao']."'")))) { //VERIFICA DUPLICIDADE
			$altera = mysql_query("UPDATE origem SET origem_descricao='".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', data='".date('Y-m-d H:i:s')."', usuario_login='".$_SESSION['login']."' where origem_id='".$_POST['id']."'");
...
//------------------------------------->> INCLUIR
		...
		if (!(mysql_num_rows (mysql_query ("SELECT origem_id FROM origem WHERE origem_descricao='".$_POST['descricao']."'")))) { // VERFICA DUPLICIDADE
			$insere = mysql_query ("INSERT origem(origem_descricao, data, usuario_login) VALUES('".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', '".date('Y-m-d H:i:s')."', '".$_SESSION['login']."')");
