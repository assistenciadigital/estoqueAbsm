...
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from origem where origem_id like '%".$_GET['id']."%' and origem_descricao like '%".$_GET['descricao']."%' order by origem_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from origem"));
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['origem_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['origem_descricao'], 1, 0, 'L');
...
$pdf->Output("origem.pdf", "I");
?> 
<script>window.open('origem.pdf'); </script>