...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
	...
	if ($_POST['id']){
	$select = mysql_query("SELECT * FROM escolaridade where escolaridade_id = '".$_POST['id']."'");	
	}else{	
	$select = mysql_query("SELECT * FROM escolaridade where escolaridade_id like '%".$_POST['id']."%' and escolaridade_descricao like '%".$_POST['descricao']."%' order by escolaridade_descricao");	
	}
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro = mysql_num_rows(mysql_query("SELECT * from escolaridade")); // Quantidade de registros pra paginação
...
function exclui(escolaridade_id){
	if(confirm ('Confirma a exclusão do registro: ' + escolaridade_id + '?')) {
		location = 'processa.php?acao=exclusao&id=' + escolaridade_id;
	}
}
...
// Captura o retorno do retornaCliente.php
    $.getJSON('../../includes/autocomplete_retorna_escolaridade.php', function(data){
    var dados = [];
    // Armazena na array capturando somente o nome do EC
    $(data).each(function(key, value) {
        dados.push(value.escolaridade_descricao);
    });
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <th width="100%" colspan="2" align="center" scope="col"><div align="midle" style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">CONSULTA ESCOLARIDADE</div></th>
  </tr>
  <tr>
...
<form action="lista.php" name="frmlista" id="frmlista" method="post" enctype="multipart/form-data">
    ...
    <tr bgcolor="#CCCCCC" style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
      ...
      <td width="80%" align="center" valign="middle" bgcolor="#CCCCCC" style="cursor:pointer" scope="col"><strong>Filtra ID: 
          <select name="id" id="id" style="width:250px; height: 20px;" onchange="atualiza();">
            <option value=""></option>
            <?php
            $select_escolaridade = mysql_query ("SELECT escolaridade_id, escolaridade_descricao from escolaridade ORDER BY escolaridade_id");
            if (mysql_num_rows ($select_escolaridade)) {
            while ($row_escolaridade= mysql_fetch_assoc($select_escolaridade)) {
            ?>
            <option value="<?php echo $row_escolaridade['escolaridade_id']?>"<?php if($_POST['id'] == $row_escolaridade['escolaridade_id']) echo "selected"?>><?php echo $row_escolaridade['escolaridade_id'].' - '.$row_escolaridade['escolaridade_descricao']?></option>
            <?php }}?>
          </select>
          Filtra  Descrição:&nbsp;
        
          <input name="descricao" id="descricao" type="text" style="alignment-adjust:middle" value="<?php echo $_POST['descricao']?>" size="50" maxlength="auto" />
      ...
      <td width="10%" align="right" valign="middle" bgcolor="#CCCCCC" style="cursor:pointer" scope="col"<?php if(($_SESSION['nivel'])<=2){ ?><?php } ?>><strong>
        <?php if(($_SESSION['nivel'])<=2){ ?>
        <a href="relatorio.php?id=<?php echo $_POST['id']?>&descricao=<?php echo $_POST['descricao']?>" target="_blank"><input type="button" name="impressao" id="impressao" value="Impressão..." style="cursor:pointer"/></a>
        <?php } ?>
      </strong></td>
      </tr>
  </table>
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
      <td width="10%" align="center" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'"><?php echo $row['escolaridade_id']?></td>
      <td width="80%" align="rigth"  bgcolor="<?php echo $cor?>" scope="col" onMouseOver="this.style.backgroundColor='#00FFFF'" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'"><?php echo $row['escolaridade_descricao']?></td>
      <td width="5%" align="center" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'" style="cursor:pointer"><?php if(($_SESSION['nivel'])<=2){ ?><a href="cadastra.php?id=<?php echo $row['escolaridade_id']?>"><img src="../../imgs/editar.png" width="20" height="20" /></a><?php } ?></td>
      <td width="5%" align="center" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'" style="cursor:pointer"><?php if(($_SESSION['nivel'])<=2){ ?><a href="javascript: exclui (<?php echo $row['escolaridade_id']?>)"> <img src="../../imgs/excluir.png" width="20" height="20" /></a><?php } ?></td>
    </tr>
  </table>
  <?php }}?>
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
