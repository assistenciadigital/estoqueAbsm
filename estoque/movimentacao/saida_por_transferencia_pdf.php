...
include "../../includes/ajustadataehora.php";
	...
	$movimento = @mysql_fetch_assoc(mysql_query("SELECT movimento.movimento_id, movimento.movimento_destino, movimento.numero_documento, movimento.data_documento, movimento.usuario_login, movimento.data, localestoque.local_descricao FROM movimento inner join localestoque on movimento.movimento_destino = localestoque.local_id where movimento.movimento_id = '".$_GET['movimento_id']."'"));	

	$lancamento = mysql_query ("SELECT historicomovimento.produto_id, historicomovimento.historicomovimento_quantidade, historicomovimento.historicomovimento_valorunitario, historicomovimento.historicomovimento_valortotal, (historicomovimento.historicomovimento_quantidade * historicomovimento.historicomovimento_valorunitario) as valor_total, produto.produto_descricao FROM historicomovimento INNER JOIN produto ON historicomovimento.produto_id = produto.produto_id where movimento_id ='".$_GET['movimento_id']."'");
...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'MOVIMENTACAO DE PRODUTOS - SAIDA POR TRANSFERENCIA - DISPENSACAO',0,1,'C');
...
$pdf->Cell(  0, 8, 'DADOS DA TRANSFERENCIA', 0, 1, 'C');		
$pdf->Cell(  0, 0, '', 1, 1);
$pdf->SetFont('Courier','',10);
$pdf->Cell(  0, 4, 'SETOR                : '.$movimento['local_descricao'], 0, 1, 'L');		
$pdf->Cell(  0, 4, 'NUMERO DOCUMENTO / ID: '.$movimento['numero_documento'], 0, 1, 'L');		
...
$pdf->Cell(  0, 0, '', 1, 1);
$pdf->SetFont('Courier','B',10);
$pdf->Cell(  0, 8, 'ITENS DA TRANSFERENCIA - PRODUTOS', 0, 1, 'C');		
...
if (mysql_num_rows ($lancamento)) {
  while ($row = mysql_fetch_assoc($lancamento)) {
		...
		$pdf->Cell( 20, 4, number_format($row['historicomovimento_valorunitario'], 2, ',', '.'), 0, 0, 'R');
...
$pdf->Output("saida_por_transferencia.pdf", "I");
?> 
<script>window.open('saida_por_transferencia.pdf'); </script>