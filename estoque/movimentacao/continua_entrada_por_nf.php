...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
	...
	$movimento = @mysql_fetch_assoc(mysql_query ("SELECT movimento.movimento_origem, movimento.numero_documento, movimento.data_documento, movimento.valor_documento, fornecedor.fornecedor_razaosocial from movimento inner join fornecedor on movimento.movimento_origem = fornecedor.fornecedor_id where movimento_id = '".$_GET['movimento_id']."'"));		
...
<form onsubmit="return validacampo(this)" action="continua_entrada_por_nf.php?movimento_id=<?php echo $_GET['movimento_id']?>" name="frmcadastro" id="frmcadastro"  method="post">
  <div style="border: 1px solid #c30; background: #ffe; font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:14px" align="center"><strong><?php echo 'Movimento: '.$_GET['movimento_id'].' - '.$movimento['fornecedor_razaosocial'].' - '.$movimento['numero_documento'].' - '.date_data($movimento['data_documento']).' - '.number_format($movimento['valor_documento'], 2, ',', '.')?></strong>
        </div><p>
  
  <div style="border: 1px solid #c30; background: #ffe; font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:14px" align="center"><p>
  <strong>ENTRADA POR NOTA FISCAL</strong> (Digite o que você está procurando e clique em Pesquisar)<p><input name="pesquisa" type="text" id="pesquisa" value="<?php echo $_POST['pesquisa']?>" size="50" maxlength="50" />&nbsp;<input type="submit" name="pesquisar" id="pesquisar" value="Pesquisar..." />
<input type="reset" name="limpar" id="limpar" value="Limpar..." onclick="limpa()"/>
<a href="entrada_por_nf.php"><input type="button" name="voltar" id="voltar" value="Voltar..." /></a><p></div><p>
</form>
<?php if ($select['produto_id']){?>
<form action="processa.php?acao=entrada_por_nf" method="post" name="frmprocessa" id="frmprocessa">
  ...
  <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0"  class="pesquisa">
      ...
      <tr>
        ...
        <td width="20%" align="left" valign="middle" bgcolor="#CCCCCC"><strong>
          ...
          ENTRADA POR NOTA FISCAL</strong></td>
        </tr>
      <tr>
      ...
      <tr>
        ...
        <td colspan="3" align="left" valign="middle" bgcolor="#CCCCCC"><input name="historico" type="text" id="historico" value="<?php echo "ENTRADA POR NOTA FISCAL - EM: ".date('d/m/Y H:m')." USUARIO: ".$_SESSION['login'] ?>" size="100" /></td>
        </tr>
      <tr>
