...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  //MONTA O CABEÇALHO DA TABELA
		...
		$this->Cell( 24,  4, "ESTQ ANTERIOR", 0, 0, 'R');
		$this->Cell( 24,  4, "ESTQ ATUAL", 0, 0, 'R');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
		...
		$estoqueatual = $row['historicomovimento_estoque_anterior'] + $row['historicomovimento_quantidade'];
		}else{
		$pdf->Cell( 20, 4, number_format($row['historicomovimento_quantidade']*-1, 0, ',', '.'), 0, 0, 'R');
		$estoqueatual = $row['historicomovimento_estoque_anterior'] - $row['historicomovimento_quantidade'];
		} 	    
		$pdf->Cell( 24, 4, number_format($row['historicomovimento_estoque_anterior'], 0, ',', '.'), 0, 0, 'R');
		$pdf->SetFont('Courier','B',8);
		$pdf->Cell( 24, 4, number_format($estoqueatual, 0, ',', '.'), 0, 0, 'R');
		$pdf->SetFont('Courier','',8);
