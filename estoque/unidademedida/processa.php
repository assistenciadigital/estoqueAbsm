...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
    ...
    $select = @mysql_fetch_assoc(mysql_query("SELECT * FROM unidade where unidade_id = '".$_POST['id']."'"));
...
<?php
	...
	if(($_SESSION['nivel'])<=2){
			mysql_query ("DELETE FROM unidade WHERE unidade_id='".$_GET['id']."'");
...
//------------------------------------->> ALTERAR	
	...
	if(($_SESSION['nivel'])<=2){
	if (!(mysql_num_rows (mysql_query ("SELECT unidade_id FROM unidade WHERE unidade_descricao = '".$_POST['descricao']."'")))) { //VERIFICA DUPLICIDADE
			$altera = mysql_query("UPDATE unidade SET unidade_descricao='".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', data='".date('Y-m-d H:i:s')."', usuario_login='".$_SESSION['login']."' where unidade_id='".$_POST['id']."'");
...
//------------------------------------->> INCLUIR
		...
		if (!(mysql_num_rows (mysql_query ("SELECT unidade_id FROM unidade WHERE unidade_descricao='".$_POST['descricao']."'")))) { // VERFICA DUPLICIDADE
			$insere = mysql_query ("INSERT unidade(unidade_descricao, data, usuario_login) VALUES('".trim(addslashes(htmlentities(strtoupper($_POST['descricao']))))."', '".date('Y-m-d H:i:s')."', '".$_SESSION['login']."')");
