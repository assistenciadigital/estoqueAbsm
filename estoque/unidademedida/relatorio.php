...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from unidade where unidade_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from unidade where unidade_id like '%".$_GET['id']."%' and unidade_descricao like '%".$_GET['descricao']."%' order by unidade_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from unidade"));
...
class PDF extends FPDF
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'RELACAO DE UNIDADE MEDIDA',0,1,'C');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['unidade_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['unidade_descricao'], 1, 0, 'L');
...
?> 
<script>window.open('unidade.pdf'); </script>