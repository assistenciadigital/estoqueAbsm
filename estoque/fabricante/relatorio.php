...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from fabricante where fabricante_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from fabricante where fabricante_id like '%".$_GET['id']."%' and fabricante_descricao like '%".$_GET['descricao']."%' order by fabricante_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from fabricante"));
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['fabricante_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['fabricante_descricao'], 1, 0, 'L');
...
$pdf->Output("fabricante.pdf", "I");
?> 
<script>window.open('fabricante.pdf'); </script>