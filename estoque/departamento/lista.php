...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
	...
	if ($_POST['id']){
	$select = mysql_query("SELECT * FROM departamento where depto_id = '".$_POST['id']."'");	
	}else{	
	$select = mysql_query("SELECT * FROM departamento where depto_id like '%".$_POST['id']."%' and depto_descricao like '%".$_POST['descricao']."%' order by depto_descricao");	
	}
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro = mysql_num_rows(mysql_query("SELECT * from departamento")); // Quantidade de registros pra paginação
...
function exclui(depto_id){
	if(confirm ('Confirma a exclusão do registro: ' + depto_id + '?')) {
		location = 'processa.php?acao=exclusao&id=' + depto_id;
	}
}
...
// Captura o retorno do retornaCliente.php
    $.getJSON('../../includes/autocomplete_retorna_departamento.php', function(data){
    var dados = [];
    // Armazena na array capturando somente o nome do EC
    $(data).each(function(key, value) {
        dados.push(value.depto_descricao);
    });
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <th width="100%" colspan="2" align="center" scope="col"><div align="midle" style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">CONSULTA DEPARTAMENTO</div></th>
  </tr>
  <tr>
...
<form action="lista.php" name="frmlista" id="frmlista" method="post" enctype="multipart/form-data">
    ...
    <tr bgcolor="#CCCCCC" style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
      ...
      <td width="80%" align="center" valign="middle" bgcolor="#CCCCCC" style="cursor:pointer" scope="col"><strong>Filtra ID: 
          <select name="id" id="id" style="width:250px; height: 20px;" onchange="atualiza();">
            <option value=""></option>
            <?php
            $select_departamento = mysql_query ("SELECT depto_id, depto_descricao from departamento ORDER BY depto_id");
            if (mysql_num_rows ($select_departamento)) {
            while ($row_departamento= mysql_fetch_assoc($select_departamento)) {
            ?>
            <option value="<?php echo $row_departamento['depto_id']?>"<?php if($_POST['id'] == $row_departamento['depto_id']) echo "selected"?>><?php echo $row_departamento['depto_id'].' - '.$row_departamento['depto_descricao']?></option>
            <?php }}?>
          </select>
          Filtra  Descrição:&nbsp;
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
      <td width="10%" align="center" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'"><?php echo $row['depto_id']?></td>
      <td width="80%" align="rigth"  bgcolor="<?php echo $cor?>" scope="col" onMouseOver="this.style.backgroundColor='#00FFFF'" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'"><?php echo $row['depto_descricao']?></td>
      <td width="5%" align="center" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'" style="cursor:pointer"><?php if(($_SESSION['nivel'])<=2){ ?><a href="cadastra.php?id=<?php echo $row['depto_id']?>"><img src="../../imgs/editar.png" width="20" height="20" /></a><?php } ?></td>
      <td width="5%" align="center" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'" style="cursor:pointer"><?php if(($_SESSION['nivel'])<=2){ ?><a href="javascript: exclui (<?php echo $row['depto_id']?>)"> <img src="../../imgs/excluir.png" width="20" height="20" /></a><?php } ?></td>
    </tr>
  </table>
  <?php }}?>
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
