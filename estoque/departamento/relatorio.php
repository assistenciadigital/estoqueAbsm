...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from departamento where depto_id='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from departamento where depto_id like '%".$_GET['id']."%' and depto_descricao like '%".$_GET['descricao']."%' order by depto_descricao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from departamento"));
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	$pdf->Cell( 14, 4, $row['depto_id'], 1, 0, 'C');
	$pdf->Cell(122, 4, $row['depto_descricao'], 1, 0, 'L');
...
$pdf->Output("departamento.pdf", "I");
?> 
<script>window.open('departamento.pdf'); </script>