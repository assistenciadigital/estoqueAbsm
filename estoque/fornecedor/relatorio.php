...
//CONECTA COM O MYSQL
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from convenio_empresa where id ='".$_GET['id']."'");
}else{
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from fornecedor where fornecedor_id like '%".$_GET['id']."%' and fornecedor_razaosocial like '%".$_GET['razao']."%' order by fornecedor_razaosocial");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from fornecedor"));
...
function NbLines($w,$txt)
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'RELACAO DE FORNECEDORES',0,1,'C');
	  ...
	  //MONTA O CABEÇALHO DA TABELA
		...
		$this->Cell( 33, 4, "INSCRICAO CPF/CNPJ", 1, 0, 'L');
		$this->Cell(88, 4, "RAZAO SOCIAL", 1, 0, 'L');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	  ...
	  $pdf->Row(array($row['fornecedor_id'], $row['fornecedor_inscricao'], $row['fornecedor_razaosocial'], $row['fornecedor_endereco'].' '.$row['fornecedor_numero'].' '.$row['fornecedor_complemento'].' '.$row['fornecedor_bairro'].' '.$row['fornecedor_cidade'].' '.$row['fornecedor_uf'].' '.$row['fornecedor_cep'].' '.$row['fornecedor_telefone'].' '.$row['fornecedor_celular'].' '.$row['fornecedor_fax'].' '.$row['fornecedor_email'], $row['data_formatada'], $row['usuario_login']));
	  }
}
...
$pdf-> Cell(0,4, number_format(($qtde_pesquisado), 0, ',', '.').' de '.number_format(($qtde_registro), 0, ',', '.')." Registro(s) [ID: ".$id." Razao Contendo: ".$razao."]",1,1,'C');
$pdf->SetFont("Courier", '', 8);
...
$pdf->Output("fornecedor.pdf", "I");
?> 
<script>window.open('fornecedor.pdf'); </script>