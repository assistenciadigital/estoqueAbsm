...
if ($_GET['id']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from produto where produto_id = '".$_GET['id']."' order by produto_id");	
	}else if ($_GET['depto']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from produto where depto_id like '%".$_GET['depto']."%' and grupo_id like '%".$_GET['grupo']."%' and produto_descricao like '%".$_GET['descricao']."%' order by produto_descricao");	
	}else if ($_GET['grupo']){
	$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from produto where depto_id like '%".$_GET['depto']."%' and grupo_id like '%".$_GET['grupo']."%' and produto_descricao like '%".$_GET['descricao']."%' order by produto_descricao");	
	}else{
...
$pdf-> SetFont("Courier", 'B', 8);
$pdf-> Cell(180,4, number_format(($qtde_pesquisado), 0, ',', '.').' de '.number_format(($qtde_registro), 0, ',', '.')." Registro(s) [ID:".$id." Depto:".$depto." Grupo:".$grupo." Descricao: ".$descricao."]",1,0,'C');
$pdf-> Cell(17,4,number_format(($soma_produto_estoque), 0, ',', '.'),1,0,'R');
...
?> 
<script>window.open('produtos.pdf'); </script>