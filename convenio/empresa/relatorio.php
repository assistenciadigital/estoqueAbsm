...
$select = mysql_query("select *, date_format(`data`,'%d/%m/%Y %H:%i') as `data_formatada` from convenio_empresa where id like '%".$_GET['id']."%' and razao like '%".$_GET['razao']."%' order by razao");
}

	// Quantidade de registros pra paginação
	$qtde_pesquisado = mysql_num_rows($select);
	$qtde_registro   = mysql_num_rows(mysql_query("SELECT * from convenio_empresa"));
...
function NbLines($w,$txt)
{
   ...
   function Header() {
	  ...
	  $this->Cell(0,4,'RELACAO DE CONVENIO - EMPRESA',0,1,'C');
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	  ...
	  $pdf->Row(array($row['id'], $row['inscricao'], $row['razao'], $row['endereco'].' '.$row['numero'].' '.$row['complemento'].' '.$row['bairro'].' '.$row['cidade'].' '.$row['uf'].' '.$row['cep'].' '.$row['fone'].' '.$row['fax'].' '.$row['email'], $row['data_formatada'], $row['usuario_login']));
	  }
}
...
?> 
<script>window.open('empresa.pdf'); </script>