...
if(($_SESSION['login']) AND ($_SESSION['nivel'])){
	...
	$encaminha = mysql_query("SELECT * FROM convenio_encaminha where titular like '%".$_POST['titular']."%'");
	
	$qtde_pesquisado = mysql_num_rows($encaminha);
	$qtde_registro = mysql_num_rows(mysql_query("SELECT * from convenio_encaminha")); // Quantidade de registros pra paginação	
?>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <th width="100%" colspan="2" align="center" scope="col"><div align="midle" style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">CONVENIOS - AGENDAMENTO</div></th>
  </tr>
  <tr>
...
<form action="lista.php" name="frmlista" id="frmlista" method="post" enctype="multipart/form-data">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">    
    <tr bgcolor="#CCCCCC" style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
      ...
      <th width="70%" align="center" valign="middle" bgcolor="#CCCCCC" style="cursor:pointer" scope="col">
	  
	  <?php  $select_titular = @mysql_fetch_assoc(mysql_query("SELECT id, nome FROM convenio_empregado WHERE id = '".$_GET['titular']."'"));
				?>
	  </th>
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">    
    <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
      ...
      <td width="10%" align="center"  bgcolor="#00FFFF" scope="col"><strong>Guia / Data</strong></td>
      <td width="10%" align="center" valign="middle"  bgcolor="#00FFFF" scope="col">Dt Agenda<br/>Dt Atende</td>
      <td width="35%" align="left" valign="middle"  bgcolor="#00FFFF" scope="col"><strong>Titular</strong></td>
      <td width="20%" align="left"  bgcolor="#00FFFF" scope="col"><strong>Especialidade</strong></td>
      <td width="20%" align="center"  bgcolor="#00FFFF" scope="col"><strong>Opções</strong></td>
    </tr>
  </table>
  
  <?php 
  //<div style="color:#009; width:100%; height:303; overflow: auto; vertical-align: left;">   
  if (mysql_num_rows ($encaminha)) {
	  while ($row = mysql_fetch_assoc($encaminha)) {
	  $titular = @mysql_fetch_assoc(mysql_query("SELECT id, nome FROM convenio_empregado WHERE id = '".$row['titular']."'"));
      $dependente = @mysql_fetch_assoc(mysql_query("SELECT  id, nome FROM convenio_dependente WHERE id = '".$row['dependente']."'"));
	  $especialidade = @mysql_fetch_assoc(mysql_query("SELECT  especialidade_descricao FROM especialidade WHERE especialidade_id = '".$row['especialidade']."'"));
  	        $i++;
    		if ($i%2) $cor = '#F0F0F0';
				else $cor = '#FFFFFF';
  ?>
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
      ...
      <td width="10%" align="center"  bgcolor="<?php echo $cor?>" scope="col" onMouseOver="this.style.backgroundColor='#00FFFF'" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'"><?php echo $row['numero_guia'].'<br/>'.date_data($row['data_guia'])?></td>
      <td width="10%" align="center" valign="middle" bgcolor="<?php echo $cor?>" scope="col" onMouseOver="this.style.backgroundColor='#00FFFF'" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'"><?php echo date_data($row['data_agenda']).' '.$row['hora_agenda'].'<br/>'.date_data($row['data_atende']).' ' .$row['hora_atende']?></td>
      <td width="35%" align="left" valign="middle" bgcolor="<?php echo $cor?>" scope="col" onMouseOver="this.style.backgroundColor='#00FFFF'" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'"><?php echo '<strong>'.$titular['nome'].'</strong><br/>'.$dependente['nome']?><br/></td>
      <td width="20%" align="left" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'"><?php echo $row['tipo_especialidade'].'<br/>'.$especialidade['especialidade_descricao']?></td>
      <td width="5%" align="center" bgcolor="<?php echo $cor?>" style="cursor:pointer" scope="col" onMouseOver="this.style.backgroundColor='#00FFFF'" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'"><?php if(($_SESSION['nivel'])<=2){?>
        <?php if ($row['data_atende']){?>
        <img src="../../imgs/Ok.png" width="30" height="31" alt=""/>
        <?php }else{?>
        <a href="../atendimento/cadastra.php?titular=<?php echo $row['titular']?>&id=<?php echo $row['id']?>"><img src="../../imgs/atencao.png" width="30" height="25" alt=""/></a>
		<?php }?>
      <?php } ?></td>
      <td width="5%" align="center" bgcolor="<?php echo $cor?>" style="cursor:pointer" scope="col" onMouseOver="this.style.backgroundColor='#00FFFF'" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'"><?php if(($_SESSION['nivel'])<=2){ ?>
        <a href="ficha.php?titular=<?php echo $row['titular']?>&id=<?php echo $row['id']?>" target="_blank"><img src="../../imgs/i_ficha.png" width="30" height="31" /></a>
      <?php } ?></td>
      <td width="5%" align="center" bgcolor="<?php echo $cor?>" style="cursor:pointer" scope="col" onMouseOver="this.style.backgroundColor='#00FFFF'" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'"><?php if(($_SESSION['nivel'])<=2){ if(!$row['data_atende']){?>
        <a href="cadastra.php?titular=<?php echo $row['titular']?>&id=<?php echo $row['id']?>"><img src="../../imgs/editar.png" width="20" height="20" /></a>
      <?php }} ?></td>
      <td width="5%" align="center" bgcolor="<?php echo $cor?>" scope="col" onMouseOut="this.style.backgroundColor='<?php echo $cor?>'" onMouseOver="this.style.backgroundColor='#00FFFF'" style="cursor:pointer"><?php if(($_SESSION['nivel'])<=2){ if (!$row['data_atende']){ ?>
  ...
  <table width="100%" border="0" align="center" cellpadding="1" cellspacing="1">
    <tr style="font-family:'Trebuchet MS', Arial, Helvetica, sans-serif; font-size:12px">
      <td align="center" valign="middle" bgcolor="#FFFFFF"><strong>Registro(s): <?php echo number_format(($qtde_pesquisado), 0, ',', '.')?></strong></td>
    </tr>
  </table>
