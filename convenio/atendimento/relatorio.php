...
$select = mysql_query("select convenio_empregado.nome as titular, convenio_dependente.nome as dependente, convenio_encaminha.id, convenio_encaminha.numero_guia, convenio_encaminha.data_guia, convenio_encaminha.tipo_especialidade, especialidade.especialidade_descricao as especialidade, profissional_nome as profissional, convenio_encaminha.descricao, convenio_encaminha.data_agenda, convenio_encaminha.hora_agenda, convenio_encaminha.observacao, convenio_encaminha.data, convenio_encaminha.usuario_login from (((convenio_encaminha INNER JOIN convenio_empregado ON convenio_empregado.id = convenio_encaminha.titular) LEFT JOIN convenio_dependente ON convenio_dependente.id = convenio_encaminha.dependente) LEFT JOIN profissional ON  profissional.profissional_id = convenio_encaminha.profissional) LEFT JOIN especialidade ON  especialidade.especialidade_id = convenio_encaminha.especialidade where convenio_encaminha.titular = '".$_GET['titular']."' order by convenio_encaminha.data_guia");
...
if (mysql_num_rows ($select)) {
  while ($row = mysql_fetch_assoc($select)) {
	  ...
	  $pdf->Row(array($row['id'], $row['numero_guia']."\n".date_data($row['data_guia']), date_data($row['data_agenda'])."\n".$row['hora_agenda'], $row['titular']."\n".$row['dependente'], $row['profissional']."\n".$row['tipo_especialidade'].': '.$row['especialidade']." (".$row['descricao'].') '."\n".$row['observacao'], datetime_datatempo($row['data']), $row['usuario_login']));
	  }
}
...
$pdf->Output("relacao_atendimento.pdf", "I");
?> 
<script>window.open('relacao_atendimento.pdf'); </script>